...
function requireAdmin(req, res, next) {
  if (!ADMIN_TOKEN) return res.status(401).json({ error: "unauthorized", reason: "ADMIN_TOKEN not set" });
...
app.get("/api/debug/db", async (_req, res) => {
  try {
    ...
    if (!supabase) return res.json({ ok: true, projectRef, supabaseUrl: SUPABASE_URL, productsCount: 0, error: null });
    const { count, error } = await supabase.from("products").select("*", { count: "exact", head: true });
    return res.json({ ok: true, projectRef, supabaseUrl: SUPABASE_URL, productsCount: count || 0, error: error ? error.message : null });
  } catch (e) {
...
app.get("/api/products", async (_req, res) => {
  ...
  try {
    const { data, error } = await supabase.from("products").select("*").order("id", { ascending: false }).limit(200);
